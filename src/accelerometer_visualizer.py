...
def quick_analysis(data):
    ...
    for task, label in viz.get_task_list():
        participants = viz.get_participants(task)
        print(f"📋 {label}:")
        ...
        for pid, name in participants:
            df = viz.get_data(task, pid)
            if not df.empty:
                duration = df['time_relative'].max()
                samples = len(df)
                total_samples += samples
                total_duration += duration
                print(f"   - {name}: {samples:,} samples, {duration:.1f}s")
...
def export_summary_stats(data, filename="accelerometer_stats.csv"):
        ...
        for pid, name in participants:
            ...
            for axis in ['x', 'y', 'z', 'magnitude']:
                ...
                all_stats.append({
                    ...
                    'max': values.max(),
                    'samples': len(values),
                    'duration_sec': df['time_relative'].max()
                })
