...
class SlopeGraphVisualizer:
    ...
    def create_slope_graph(self, df: pd.DataFrame, metric: str, task_name: str, 
        ...
        for _, row in metric_data.iterrows():
            ...
            # Add participant labels on the right
            ax.text(x_challenge + 0.05, challenge_val, 
                   f'P{participant_id} ({participant_name})\n{percent_change:+.1f}%',
        ...
        # Add task-specific cognitive task labels
        cognitive_tasks = {
            'step_count': 'Task Switching',
            'sit_to_stand': 'Stroop Task', 
            'water_task': 'Verbal Fluency (Fruits)'
        }
        
        if task_name in cognitive_tasks:
            ax.text(x_challenge, ax.get_ylim()[1], f'+ {cognitive_tasks[task_name]}',
        ...
        n_participants = len(metric_data)
        mean_change = metric_data['change'].mean()
        mean_percent_change = metric_data['percent_change'].mean()
        
        title = f'{task_name.replace("_", " ").title()} - {metric_label}\n'
        title += f'Mean Change: {mean_change:+.2f} {unit} ({mean_percent_change:+.1f}%) | n={n_participants}'
        ax.set_title(title, fontsize=14, fontweight='bold', pad=20)
        
        # Add hypothesis box
        hypothesis_color = 'lightcoral' if mean_change > 0 and task_name in ['step_count', 'sit_to_stand'] else 'lightgreen'
        if task_name == 'water_task' and mean_change < 0:
            hypothesis_color = 'lightgreen'
        
        ax.text(0.02, 0.98, f'Hypothesis: {hypothesis}',
               transform=ax.transAxes, fontsize=10, fontweight='bold',
               verticalalignment='top',
               bbox=dict(boxstyle='round,pad=0.4', facecolor=hypothesis_color, alpha=0.8))
        
        # Add legend
        legend_elements = [
            plt.Line2D([0], [0], color=self.colors['increase'], linewidth=3, 
                      label='Performance Deterioration', alpha=0.8),
            plt.Line2D([0], [0], color=self.colors['decrease'], linewidth=3,
                      label='Performance Improvement', alpha=0.8),
            plt.Line2D([0], [0], color=self.colors['neutral'], linewidth=3,
                      label='Minimal Change', alpha=0.6)
        ]
